...
const { extractUserFromHeaders } = require('../utils/auth');
...
const userHandler = {
    ...
    async getUserDetails(req, res, next) {
        const jwtUser = extractUserFromHeaders(req.headers);
        ...
        if (req.body) {
            try {
                ...
                if (item && item.enable === true) {
                    ...
                    if (passwordVerify) {
                        const accessToken = await jwt.sign({ id: item._id, user: item.email }, process.env.ACCESS_TOKEN_SECRET)
                        const refreshToken = await jwt.sign({ id: item._id, user: item.email }, process.env.REFRESH_TOKEN_SECRET)
